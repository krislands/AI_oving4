...
def d_L(w,x_train,y_train):
    if y_train == 0:
        val= logistic_wx(w,x_train)*exp(-inner(w,x))*pow(logistic_wx(w,x),2) #no y value
    else:        
        val= (logistic_wx(w,x_train)-y_train)*exp(-inner(w,x))*pow(logistic_wx(w,x),2) #y value
    return val
    ...
    for it in xrange(niter):
        for i in xrange(dim):
            ...
            for n in xrange(num_n):
                update_grad+=(-logistic_wx(w,x_train[n])+y_train[n])+d_L(w,x_train[n],y_train[n])# something needs to be done here
